...
toolRegistry.register(
    ...
    // 回退路径无 PTY → 强制走审批弹窗（语义不丢；见 docs/pty-research.md §7 #14）。
    ...
    // 规则不能推翻「沙盒脱壳」权限的显式禁止（与 Rust `apply_rule_clearance` 对齐）
    ...
    if (decision === 'ask') {
      ...
      // 终端内确认请求：Rust 原生路径会据此下发 presentation；回退路径无 PTY → 这里仍是弹窗
