 ...
 *
 * 语义：**全量替换** —— 模型每次都必须传完整清单，空数组 = 清空。不做 merge/增量：增量需要保存
 * 「上一版」状态，而状态只在消息历史里，工具执行器读不到，反而会引入一份影子状态。
 *
 * 本工具不保存任何状态：清单随 tool_result 消息的 `content`（给模型）+ `uiData`（给 UI）一起落库，
 * UI 侧由 `pickCurrentTodos()` 从消息里派生「唯一的那份清单」。
 *
 * ⚠️ 已原生化：Rust 引擎走 `native_tools/plan/todo_write.rs`（默认路径），本文件是回退路径；两侧语义
 * 必须完全一致（铁律 1）：改这里的校验文案 / `content` 渲染 / `uiData` 结构，必须同步改 Rust 侧
