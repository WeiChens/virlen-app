 ...
 * 规范化并校验规则文件路径。
 *
 * 允许：工作目录内的相对路径（`AGENTS.md`、`.cursor/rules.md`、`.\docs\MEMORY.md`）
 * 拒绝：绝对路径 / 盘符 / `~` / 任意 `..` 段 / 含空字节 / 空值 / 超过 200 字符
 * 顺带归一化：反斜杠 → `/`，丢弃 `.` 段与重复斜杠（因此 `./AGENTS.md` 可用）
 *
 * ⚠️ 这是**唯一**的路径准入实现：编辑弹窗在用户输入时用它驳回，读取时再用它兜底
 * （老版本存下来的脏配置）。两边必须是同一个函数，否则「界面上能存、运行时读不到」。
 *
