/**
 * StepProvider — 引导第三步：配置模型服务商
 *
 * 选择服务商 → 填写 API Key（自定义还需 Base URL）→ 填写/自动获取模型 ID，
 * 校验通过后注册 Provider 并完成初始化。
 */
...
function StepProvider({ onBack, onComplete }: Props) {
  ...
  async function handleFetchModels() {
    ...
    try {
      ...
      const msg = tpl('获取失败: $__error__', {
        error: err.message || String(err),
      })
      // 内联错误：与字段就近展示，避免只靠 toast 一闪而过
      setError(msg)
      showToast(msg)
    } finally {
      setFetching(false)
    ...
    <div className="setup-provider">
      ...
      {isCustom && (
        <div className="api-key-section">
          <label className="api-key-label" htmlFor="setup-base-url">
            Base URL
          </label>
          <input
            id="setup-base-url"
            className="api-key-input has-no-toggle"
            type="url"
      ...
      {template && (
        <div className="api-key-section">
          <label className="api-key-label" htmlFor="setup-api-key">
      ...
      {template && (
        <div className="api-key-section">
          <label className="api-key-label" htmlFor="setup-model-id">
            {t('模型 ID')}
          </label>
      ...
      {!canSubmit && !error && (
        <p className="setup-form-hint">
          {t('请选择服务商，并填写 API Key 与模型 ID 后即可完成')}
        </p>
      )}
