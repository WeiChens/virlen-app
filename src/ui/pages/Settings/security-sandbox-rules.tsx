 ...
 *
 * 维护一组「命中即强制以『不使用沙盒』方式执行」的命令规则：沙盒开启时，命中规则的命令
 * 免去「沙盒脱壳」授权，并被引擎直接改为无沙盒执行（AI 不必显式传 `sandbox:"off"`）。
 * 匹配逻辑只有一份，在 `@/domain/security/sandbox-ignore-rules`；判定入口是
 * `infrastructure/tools/execute/*`（TS 路径）与内部交互 `sandbox_rule_check`（Rust 原生执行时回调 JS）。
 *
 * ⚠️ 规则**只**免除「沙盒脱壳」授权：命令本身的风险审批（终端正常/安装/危险命令、
 *    脚本执行）仍按「权限管理」里的三态设置走；`deny` 与只读沙盒不受规则影响。
 *
