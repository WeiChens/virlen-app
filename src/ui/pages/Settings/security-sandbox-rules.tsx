 ...
 * 匹配逻辑只有一份，在 `@/domain/security/sandbox-ignore-rules`；Rust 与 CLI 的判定在
 * `virlen-core/src/security/`（同一份 golden 契约收敛）。
 *
 * ⚠️ 规则只免除「沙盒脱壳」授权：命令本身的风险审批（终端正常/安装/危险命令、脚本执行）
 *    仍按「权限管理」里的三态设置走；`deny` 与只读沙盒不受规则影响。
 *
