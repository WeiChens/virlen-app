...
//!
//! 基于本地文件存储：知识库的创建 / 删除 / 列表、文档的添加 / 删除 / 列表、语义检索（向量相似度搜索）。
//!
//! ⚠️ 数据目录由宿主注入（`HostEnv::data_dir()`）而不是从 `tauri::AppHandle` 取 —— 这样 CLI 与 GUI 落在
//! 同一目录、读写同一份知识库。Tauri 命令在 `virlen-app` 的 `src/commands/rag.rs`。
