...
//!
//! 为什么单独一个模块：`list_models` 只在「配供应商」这一刻用得上，引擎跑一轮对话完全不需要它；加进
//! trait 就得连 `BridgedProvider`（Gemini 等走 JS 桥的）一起实现一遍，而且这里的能力是面向调用方
//!（CLI 向导 / 诊断命令）的。
//!
//! 两个函数都是 `src/infrastructure/provider/{openai,anthropic}.ts` 里同名逻辑的移植（铁律 1）：
//! `list_models` ↔ `listModels()`（`GET {base}/models`）、`verify_connection` ↔ `validateApiKey()`
//!（发一条 `ping` 的最小对话，`max_tokens: 1`）。
//!
//! ⚠️ anthropic 的 `list_models` 故意不拼 basePath：Anthropic 的模型列表在根域上，不在 `/v1` 下面，
//! 这里与 TS（`new URL(baseUrl)` 取 `protocol//host`）逐字对齐。
...
/// 列表拉取的超时上限。
///
/// `reqwest::Client::new()` 默认没有超时 —— 配置向导里一个不可达的 Base URL 会把用户永久卡在
/// 「正在拉取…」。20s 足够一次正常的 /models 往返，也短到用户不会以为死机。
...
///
/// 与 GUI「验证 API Key」同一条路径、同一个 `max_tokens: 1`，因此 CLI 里能从 401/404/额度错误里拿到
/// 与桌面端一致的判定。成功时返回助手回答的纯文本（一般只有 1 个 token，可能为空串）。
///
/// 这是真实计费调用（虽然只花 1 个 token）；向导里应作为可选步骤并提前说明。
