...
///
/// 当前只有 Windows 用例（`execute_command/tests.rs` 的进程树 kill 验证，同样带 Windows 门禁）
/// 在调用它 → 非 Windows 下这个函数没有调用者。它本身是「两平台各自实现」的探针，非 Windows
/// 分支（`kill -0`）是给今后 Linux 用例留的，故按平台 allow 而不是删掉。
