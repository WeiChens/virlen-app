...
//!
//! 与 `list_messages` 同源（同一 `SessionRepo` 查询），只是定位方式不同：给定锚点（id 优先，可用 seq）与
//! 相对窗口 `[-10,0]` / `[0,10]` / `[-5,5]`，返回该窗口内各条消息的正文。
//!
//! 硬性约束：只覆盖「已压缩区间」（触及边界即停止，并提示后续内容已在上下文中）；深度思考永不返回；
//! 工具调用只给「工具名 + 参数摘要」（≤100 字符，由 `session_db` 侧截断）；单条正文 ≤ 4000 字符、单次
//! 窗口 ≤ 21 条、单次输出 ≤ 30000 字符。
//!
