...
//! `compose_system_prompt()` ↔ `composeSystemPrompt()`、`build_project_rules_prompt()` ↔
//! `domain/agent/project-rules.ts::buildProjectRulesPrompt`。
//!
//! 调用方：CLI（生产，`session_rt/resources.rs::build_system_prompt` —— headless 下没有前端，提示词
//! 只能在这里拼）、GUI（组装仍在 TS `services/agent-service.ts::assembleAgentPrompt`，本模块对它是
//! 「golden 比对的另一半」）。
//!
//! ## ⚠️ CLI 与 GUI 的已知差异（golden 守不住这一层，别误以为「已完全对齐」）
//!
//! 顺序与分隔符由 golden 逐字节锁定，两侧一致；但喂进来的 `PromptParts` 两侧不同 —— golden 用的是
//! 固定输入，它守的是「组装规则」，守不住「输入内容」：环境信息 CLI 给不出 OS 版本号与工具版本
//! （headless 不探测）；**项目规则 CLI 目前直接塞文件原文**（未经 `build_project_rules_prompt`，与
//! `PromptParts::project_rules` 的契约不符 —— 待确认是否应一并包装）；角色 / 身份 / 性格 / 技能 CLI
//! 不注入（headless 没有这些输入，是「没有数据」而非「另一份实现」）。
//!
//! 结论：环境信息无法逐字节同源，只能保证「格式与位置一致」。改这里请同步 `docs/rust-engine.md` §12.2。
...
mod tests {
    ...
    // ── 固定输入 ──
    // ⚠️ 必须与 TS 侧 `src/tests/domain/compose-prompt-golden.test.ts` 的常量逐字一致，否则 golden
    //    比对失去意义（两侧输入不同 = 比对的是别的东西）。
