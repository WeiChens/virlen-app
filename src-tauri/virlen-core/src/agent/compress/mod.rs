...
//!
//! 与 TS `src/domain/engine/compress-context.ts`（`ai` 摘要）+ `compress-raw.ts`（正文压缩）
//! **同语义**（铁律 1：改语义要两侧同步）。差异如实列在文件末尾。
//!
//! ## 两种模式
//!
//! - [`CompressMode::Ai`]：一次**非流式**模型调用（提示词 `prompts::COMPRESS_CONTEXT`）。
//!   最省 token，但慢、且本身要花钱；
//! - [`CompressMode::Raw`]：**正文压缩**，纯本地渲染（[`raw::build_raw_summary`]），
//!   毫秒级、零消耗；正文一字不删，只丢深度思考并省略超长工具输出。
//!
//! ## 产物与「压缩怎么生效」
//!
//! 产物**只有一条** `role = "summary"` 消息，由调用方**追加**到会话末尾。
//! 生效机制不在本模块：请求组装时 `provider::blocks::slice_messages` 会丢掉最后一个
//! summary 之前的全部消息 —— 所以 summary 正文必须**自包含**。
//!
//! ⚠️ 旧消息**仍然留在库里**：模型侧查询工具（`list_messages` / `read_messages`）靠它们
...
//!    本模块只有 [`estimate_tokens`] 的 CJK 感知**粗估** —— 因此 `ui_data.contextTokens`
