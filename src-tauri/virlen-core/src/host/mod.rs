...
//!
//! 与 `virlen-app` 的 `TauriHost` 只差「资源目录怎么找」「数据目录在哪」，不含任何业务逻辑；两者都在构造期
//! 显式注入（`Arc<dyn HostEnv>`），没有隐式全局依赖。
//!
//! ⚠️ 本模块（`virlen-core`）不得出现 `tauri::` —— GUI 的 `TauriHost` 住在 `virlen-app/src/host/`。
...
/// 进程级默认宿主（CLI 语义）。
///
/// ⚠️ 只给「没有注入点」的边缘路径用（如 GUI 侧不经过 `AgentEngine` 的 `pty_run_command`）；生产主路径（Agent
/// 引擎 → `NativeToolCtx.host`）一律走显式注入，不用全局单例 —— 否则 CLI / GUI 的初始化顺序会变成隐式依赖，
/// 单测也无法并行。
