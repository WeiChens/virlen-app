...
//!
//! 换句话说：GUI 与 CLI 的行为差异只允许来自「宿主注入」，不允许来自「两份实现」（铁律 1 在
//! crate 层面的落地 —— 以前靠注释约定，现在由编译器强制）。
//!
//! ## 模块
//!
//! - [`agent`]：Agent 引擎（Rust 原生聊天循环）
//! - [`session_db`]：会话 + 应用配置持久化（SQLite，WAL，单写连接）
//! - [`sandbox`]：跨平台沙盒（Windows：Job Object + 受限令牌 + ACL；Linux：Landlock）
//! - [`security`]：安全域（「忽略沙盒命令」规则的权威匹配 + `js` 规则内嵌求值）
...
//!
//! ⚠️ headless CLI 入口不在本 crate：命令实现（`config` / `run` / `list-session` / `list-agent`）
//! 与（规划的）TUI 都在 `virlen-cli`（它只依赖本 crate）—— 这样 core 只管「引擎 + 持久化」，入口
//! 形态可以独立演进。
