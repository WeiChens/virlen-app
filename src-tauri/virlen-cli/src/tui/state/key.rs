...
impl UiState {
    ...
    ///
    /// 两种交互的键盘模型刻意不同：
    /// - 授权（`confirm_command_native`）：显式二选一 —— ←/↑ = 拒绝、→/↓ = 允许、Enter 确认当前
    ///   高亮项，默认高亮「拒绝」。普通字符一律不接受（用户此刻很可能正在打下一句消息，`y` / `n`
    ///   都不是对授权的表态）；
    /// - 选择（`user_choice` 等）：仍是行输入（不是安全闸门，序号 / 文本都可用）。
    ///
    /// ⚠️ 历史实现把授权也做成「行输入 + 回车放行」（空白 = 允许）：用户打字时的一次误触 Enter 就
    /// 直接批准了危险命令。这条 fail-open 已被移除，回归用例见
