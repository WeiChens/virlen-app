...
//!
//! 背景（另见 `notify.rs` 模块头）：Windows 的 toast 点击有两条互不排斥的投递路径：
//! ① 进程内事件：`notify::show_owned` 自持 `NotificationHandle`，点击在进程内到达，能精确知道是
//!    哪个会话（dev / 免安装 exe 走这条）；
//! ② COM 激活：系统按清单里的 `ToastActivatorCLSID` 唤起「激活器」—— 应用没在跑时由它拉起进程
//!    （`com:ExeServer@Arguments` 带 `-ToastActivated`），正在跑时把调用投递进已注册的类对象。
//!
//! 本模块实现 ②。拿不到会话 id（toast 的 `launch` 参数一路没有来源 —— notify-rust 的 winrt 后端不
//! 写 `launch`），所以语义与「托盘左键」一致：唤醒窗口 + 切到最早那条未读。
//!
...
//!
//! ⚠️ 只有打包安装版才有清单，dev / 免安装 exe 注册了也没人来调。但注册本身无副作用（类对象只在
//! 本进程可见，不进系统注册表），因此不做环境判断，一律注册 —— 少一个「装完包才发现没生效」的坑。
...
mod windows_impl {
    ...
    impl INotificationActivationCallback_Impl for ToastActivator_Impl {
        fn Activate(
            ...
            // ⚠️ 本回调跑在 COM 的 RPC 线程上：只做非阻塞投递（tauri 会把窗口操作 marshal 回
            // 主线程），绝不在这里等待或做重活。
    ...
    /// 在一条**常驻线程**上注册类对象
    ///
    /// ⚠️ 线程必须常驻：`CoRegisterClassObject` 的注册挂在注册线程的 apartment 上，线程一退（隐式
    /// `CoUninitialize`）注册就没了；而「点击通知拉起进程」要求进程在系统给的那几秒内把类对象挂上
    /// 并一直持有，否则激活直接失败。
    ///
    /// ⚠️ 用 MTA（`COINIT_MULTITHREADED`）：MTA 下不需要消息泵，激活由 RPC 线程投递进来；放在
    /// tauri 主线程（STA）上就得依赖它的消息循环，还得和 WebView2 的 OLE 初始化抢地盘。
    ...
    mod tests {
        ...
        ///
        /// ⚠️ 拼错任何一处都会让「点击通知」静默失效（COM 激活失败不会弹到 UI），所以这里用 GUID
        /// 的 Debug 格式（大写、无花括号）反查一遍。
        #[test]
