...
mod windows_impl {
    ...
    impl INotificationActivationCallback_Impl for ToastActivator_Impl {
        fn Activate(
            ...
            // 本回调跑在 COM 的 RPC 线程上：只做非阻塞投递（窗口操作由 tauri marshal 回主线程），
            // 绝不在这里等待或做重活。
    ...
    /// 在一条**常驻线程**上注册类对象
    ///
    /// 线程必须常驻：`CoRegisterClassObject` 的注册挂在注册线程的 apartment 上，线程一退（隐式
    /// `CoUninitialize`）注册就没了，而系统给「点击拉起进程」的窗口只有几秒。
    /// 用 MTA（`COINIT_MULTITHREADED`）：不需要消息泵，激活由 RPC 线程投递；放 tauri 主线程（STA）
    /// 则要依赖它的消息循环，还得和 WebView2 的 OLE 初始化抢地盘。
    ...
    mod tests {
        ...
        /// `CLSID`（GUID 常量）与 `TOAST_ACTIVATOR_CLSID`（给人看、与清单对照的字符串）必须描述
        /// 同一个 CLSID —— ⚠️ 拼错任何一处都会让「点击通知」静默失效，所以这里用 GUID 的 Debug 格式
        /// （大写、无花括号）反查一遍。
        #[test]
