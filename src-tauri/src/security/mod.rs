...
//!
//! 目前只包含「忽略沙盒命令」规则的判定：
//! - [`rules`]：规则模型 + `text` / `regex` / `js` 三种匹配（与 TS 侧**共读 golden** 对齐）；
//! - [`js_rule`]：`js` 规则体的内嵌 QuickJS 求值（受限 runtime：无 host 函数 / 内存 / 超时）。
//!
//! ⚠️ 与 `crate::sandbox` 的分工：`sandbox` 是**执行机制**（Job Object / Landlock / 受限令牌），
//! 这里是**策略判定**（这条命令要不要免脱壳审批），不碰任何平台执行细节。
//!
//! 消费方：`agent/native_tools/execute/common/rules.rs`（Rust 引擎 + CLI 路径）。
//! TS 实现（`src/domain/security/sandbox-ignore-rules.ts`）保留给浏览器 dev / TS 引擎路径 /
//! 设置页「测试」按钮，两侧行为由 golden 契约收敛。
...
/// 键名字面量只在这里与前端各出现一次，避免「两侧各写一个字符串」而悄悄漂移
/// （配置下沉的键名约定，见 `docs/config-sink-plan.md` §6 R6）。
pub(crate) const SANDBOX_RULES_SETTINGS_KEY: &str = "sandboxIgnoreRules";

/// 从配置后端读取「忽略沙盒命令」规则（**没有前端时的读取入口**）。
///
/// 两条路径读的是**同一个** `app_settings` 键，判定实现也是同一份（本模块）：
/// - GUI（Rust 引擎）：由前端 `resolveSecurityConfig` 读同一键后随 `NativeToolSecurity`
///   下发 —— 复用同一次 IO，命令执行时零额外开销；
