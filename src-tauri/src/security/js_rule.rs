...
//! | 异常 / 超时 / OOM | 一律 `Err` → 上层按**未命中**（fail-closed，绝不放行） |
//!
//! ⚠️ **中断处理器必须总是设置**：`quickjs_runtime` 在创建 runtime 时无条件把 C 回调
//! 注册进 QuickJS，而该回调会 `unwrap()` 这个字段 —— 不设置的话，**任何**一次 JS 执行
//! 都可能 panic（跨 FFI，后果不可控）。我们用它实现超时，正好也满足了这个前置。
//!
...
//! 4. 含 `return` → 原样当函数体；
//! 5. 其他 → 先当**单表达式**（补 `return (...)`），编译不过再回退为原样语句体。
//!
//! ⚠️ 第 5 级的回退条件与 TS 有一处**实现细节差异**：TS 用 `new Function` 只编译不执行，
//! 所以它只在**编译失败**时回退；这里用 `eval_sync` 无法只编译，故**任何错误**都回退一次。
//! 结论等价 —— 运行期抛错的写法（如 `command.match(/x/)[0]`）两种写法都抛错，最终都是
//! 「未命中」，不会因此放行。
