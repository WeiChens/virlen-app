...
//! `formatSearchResults` 的输出会直接进模型上下文，两侧任何一字之差都会造成
//! 「默认引擎与回退引擎给模型的搜索结果格式不同」。收敛靠两侧共读的 golden：
...
//!
//! 已知的**字符计数口径差异**：TS 的 `String.length` 是 UTF-16 码元数，Rust 的
//! `chars().count()` 是 Unicode 标量数（emoji 在 TS 里算 2、在 Rust 里算 1）。
//! 只影响「截断阈值」附近的行为，不影响正常内容；截断本身两侧都按字符边界切，
//! 不会产生非法字节/孤立代理。
...
/// 旧实现只认大写 → 大量网页被判成「非 HTML」，`htmlToMd` 形同虚设。
