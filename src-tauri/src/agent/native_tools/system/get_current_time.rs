...
//! - `uiData` 只下发语言无关的 `{ timestamp, timezone }`，UI 侧（`GetCurrentTimeMessage`）
//!   按界面语言重建 —— 同一个工具在「Rust/TS 引擎 × 中/英界面」四种组合下都不分叉（D2）。
//!
...
///
/// ⚠️ 与 `Intl` 的对应关系（改一处必须改另一处）：
/// `weekday:'long'`→`%A`、`month:'2-digit'`→`%m`、`day:'2-digit'`→`%d`、
/// `year:'numeric'`→`%Y`、`hour:'2-digit'`→`%I`（12 小时制**补零**）、
/// `minute`/`second`→`%M`/`%S`、AM/PM→`%p`。
/// chrono 的 `%A` / `%p` 是无本地化的英文常量，与 `en-US` 一致。
