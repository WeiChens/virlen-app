...
//!
//! ⚠️ 与 TS 侧 `src/infrastructure/tools/chat/list-messages.ts` **逐字对齐**（铁律 1）。
//! 查询直接走 `SessionRepo`（SQLite），无 JS 桥往返；`repo.is_available() == false`
//! （`NoopSessionRepo`，即没有本地存储）时回与 JS 路径一致的「不可用」文案。
