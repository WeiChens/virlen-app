...
pub trait HostEnv: Send + Sync {
    ...
    ///
    /// - GUI：`[<CARGO_MANIFEST_DIR>/resources, <resource_dir>, <resource_dir>/resources]`
    /// - CLI：`[<CARGO_MANIFEST_DIR>/resources, $VIRLEN_RESOURCE_DIR, <exe_dir>/resources, <exe_dir>]`
    ///
    /// ⚠️ 两侧都以**编译期资源根**打头，是为了让开发期 / `cargo test` 与现状完全一致
    /// （现状 `resolve_models_dir` 就先探测 `CARGO_MANIFEST_DIR/resources`）。
